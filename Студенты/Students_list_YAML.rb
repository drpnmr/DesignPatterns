...
class Students_list_YAML
  ...
  def load_from_file
    ...
    students_data = YAML.load_file(file_path)  # Загружаем YAML файл
    puts "Загруженные данные: #{students_data.inspect}"  # Выводим для проверки
    
    students_data.map do |student_hash|
      # Проверьте, что student_hash действительно является хэшом, прежде чем использовать его ключи
      if student_hash.is_a?(Hash)
        Student.new(
          id: student_hash['id'],
        ...
        raise "Ошибка данных: элемент не является хэшем: #{student_hash}"
      end
    end
  end
  ...
  def save_to_file
    File.open(file_path, 'w') do |file|
      # Преобразуем объекты студентов в хэши и записываем их в YAML
